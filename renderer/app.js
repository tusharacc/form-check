...
// ── Frontend logger ───────────────────────────────────────────────────────────
// Wraps console.* with a [FormCheck] prefix and ISO timestamp so log lines
// are easy to filter in DevTools or when piped via Electron's --log flag.
const log = {
...
function renderFrame(b64) {
  const img = new Image();
  img.onload = () => {
    // Size the canvas buffer to match its CSS layout area, not the source resolution.
    // This prevents the canvas from overflowing its flex container at 1920×1080.
